...
# ---------------------------------------------------------------------------
# Tuning: bias toward a faster midgame flood with more assault bots
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Marker encoding helpers (unchanged from original)
# ---------------------------------------------------------------------------
...
class Player(base.Player):
    def __init__(self):
        ...
        # Ammo project state (one project at a time per bot)
        ...
        # Axionite-back-to-core project
        self.ax_harvester_pos: Position | None = None
        self.ax_chain_done = False
        self.last_ax_replan_round = -999
    ...
    def refined_enemy_guesses(self, ct: Controller) -> list[Position]:
        ...
        width, height, core_pos = self.width, self.height, self.core_pos
        candidates = [
            (
    ...
    def run_core(self, ct: Controller) -> None:
        ...
        # Convert any axionite sitting in the pool (1 Ax → 4 Ti)
    ...
    # -----------------------------------------------------------------------
    # Lane hub helpers (splitter at endpoint)
    # -----------------------------------------------------------------------
    ...
    def run_assault(self, ct: Controller) -> None:
        ...
        # Fire on enemy buildings we're standing on
        ...
        if self.enemy_core_pos is not None:
            # Engineers: build gunners, feed them ammo, and tap axionite back home
            if self.is_assault_engineer(ct):
                # Priority 1: maintain an ammo feed for a gunner
                if self.try_maintain_ammo_chain(ct):
                    return
                # Priority 2: tap axionite ore near enemy core back to our core
                if self.try_maintain_axionite_route(ct):
                    return
                # Priority 3: build the siege ring
                if self.run_siege(ct):
                    return
            else:
                # Non-engineers: lightweight, infrequent build checks
                current_round = ct.get_current_round()
                if current_round % 4 == (ct.get_id() % 4):
                    if self.run_siege(ct):
                        return
    ...
    def is_assault_engineer(self, ct: Controller) -> bool:
        """~1/3 of assault bots act as engineers (build/feed infrastructure)."""
        if self.ammo_gunner_pos is not None:
            return True
        if self.ax_harvester_pos is not None:
            return True
        return (ct.get_id() + (self.assigned_guess_index or 0)) % 3 == 0
    ...
    # -----------------------------------------------------------------------
    # ── AMMO CHAIN: feed titanium to siege gunners ──────────────────────────
    #
    # Strategy:
    #   1. Find a friendly gunner that has no conveyor feeding it.
    #   2. First check if an enemy conveyor carrying titanium is nearby —
    #      if so, redirect it with a bridge to the gunner's feed side (free Ti!).
    #   3. Otherwise: find the closest titanium ore, build a harvester, then
    #      lay a cardinal conveyor chain from the harvester to a valid feed
    #      side of the gunner (any side except the direction the gunner faces).
    # -----------------------------------------------------------------------
    ...
    def try_maintain_ammo_chain(self, ct: Controller) -> bool:
        ...
        if not self.ammo_project_valid(ct):
            ...
            # First: try to tap a nearby enemy titanium conveyor (free ammo source)
            if self.try_tap_enemy_conveyor(ct):
                return True

            # Then: plan a fresh harvester→gunner chain from ore
            if not self.select_ammo_project(ct):
                return False

        if self.ammo_chain_done:
            return False
        ...
        # Step 1: ensure harvester exists on the ore
        ore_building_id = (
            ct.get_tile_building_id(self.ammo_ore_pos)
            if ct.is_in_vision(self.ammo_ore_pos)
            else None
        )
        if ore_building_id is None or ct.get_entity_type(ore_building_id) != EntityType.HARVESTER:
        ...
        # Step 2: lay the conveyor chain path[0]→path[1]→...→gunner
        for index, pos in enumerate(self.ammo_path):
            ...
            if self.transport_link_ready(ct, pos, dest):
                continue
            # Need to build here; move into range first
            ...
            if self.ensure_transport_link(ct, pos, dest):
                return True
            # Can't build yet (blocked / no funds); move adjacent to try
            if current != pos:
                return self._assault_step_toward(ct, pos)
            return False  # standing here but can't build — wait

        self.ammo_chain_done = True
        return False

    # ── tap enemy conveyor carrying titanium ───────────────────────────────

    def try_tap_enemy_conveyor(self, ct: Controller) -> bool:
        """
        If there's an enemy conveyor carrying titanium within range of a
        gunner feed side, build a bridge from that conveyor to the feed side.
        Resources can flow to enemy buildings — and we built the gunner, so
        it IS our building — so this should funnel enemy titanium to our guns.
        ...
        current = ct.get_position()
        _, all_gunner_slots = self.siege_positions()

        for gunner_pos in all_gunner_slots:
            if base.building_type_at(ct, gunner_pos) != EntityType.GUNNER:
                continue
            gunner_id = ct.get_tile_building_id(gunner_pos)
            if gunner_id is None:
                continue
            if ct.get_team(gunner_id) != ct.get_team():
                continue
            if not self.gunner_needs_feed(ct, gunner_pos):
                continue

            face = ct.get_direction(gunner_id)
            # Find valid feed sides for this gunner
            feed_sides = self.feed_side_candidates(ct, gunner_pos, gunner_id)
            if not feed_sides:
                continue

            # Scan nearby enemy conveyors/splitters for titanium
            for entity_id in ct.get_nearby_buildings():
                if ct.get_team(entity_id) == ct.get_team():
                    continue
                ...
                if etype not in (EntityType.CONVEYOR, EntityType.SPLITTER, EntityType.ARMOURED_CONVEYOR):
                    continue
                try:
                    res = ct.get_stored_resource(entity_id)
                except Exception:
                    continue
                ...
                # Try bridging from enemy conveyor to each feed side
                for feed_pos in feed_sides:
                    if enemy_pos.distance_squared(feed_pos) > 9:
                        continue
                    # Check nothing blocks the destination tile
                    if not self.transport_path_tile_open(ct, feed_pos):
                        continue
                    cost = ct.get_bridge_cost()
                    if not base.can_afford(ct, cost[0], cost[1], reserve=base.reserve_assault(ct)):
                        continue
                    if current.distance_squared(feed_pos) > GameConstants.ACTION_RADIUS_SQ:
                        return self._assault_step_toward(ct, feed_pos)
                    if ct.can_build_bridge(feed_pos, gunner_pos):
                        ct.build_bridge(feed_pos, gunner_pos)
                        # Record as done — bridge will carry the flow
                        self.ammo_gunner_pos = gunner_pos
                        self.ammo_chain_done = True
                        return True
        return False

    # ── fresh harvester→gunner chain ──────────────────────────────────────
    ...
    def ammo_project_valid(self, ct: Controller) -> bool:
        ...
        if ore_id is None:
            return True  # ore tile is clear — harvester not yet built, that's fine
    ...
    def select_ammo_project(self, ct: Controller) -> bool:
        ...
        gunner_positions.sort(
            key=lambda pos: (
                0 if pos in self.primary_gunner_positions() else 1,
                current.distance_squared(pos),
            )
        )
        ...
        for gunner_pos in gunner_positions:
            ...
            for ore_pos in ct.get_nearby_tiles():
                if ct.get_tile_env(ore_pos) != Environment.ORE_TITANIUM:
                    continue
                if ore_pos.distance_squared(gunner_pos) > 100:
                    continue
                claim_owner = self.recent_ore_claim_owner(ct, ore_pos)
                if claim_owner is not None and claim_owner != self.claim_owner_token(ct):
                    continue
                ore_building = ct.get_tile_building_id(ore_pos)
                if ore_building is not None and ct.get_entity_type(ore_building) not in (
                    EntityType.HARVESTER, EntityType.MARKER
                ):
                    continue
                ...
                for stand in self.ore_start_positions(ct, ore_pos):
                    for feed_pos in feed_sides:
                        ...
                        score = (
                            ...
                            - (8 if ore_building is not None
                               and ct.get_entity_type(ore_building) == EntityType.HARVESTER
                               else 0)
                        )
                        if score < best_score:
                            best_score = score
    ...
    def gunner_needs_feed(self, ct: Controller, gunner_pos: Position) -> bool:
        ...
        for adj in base.adjacent_cardinals(gunner_pos):
            ...
            if entity_type == EntityType.BRIDGE and ct.get_bridge_target(building_id) == gunner_pos:
                return False
            if entity_type == EntityType.ARMOURED_CONVEYOR and adj.add(ct.get_direction(building_id)) == gunner_pos:
                return False
        return True
    ...
    def feed_side_candidates(self, ct: Controller, gunner_pos: Position, gunner_id: int) -> list[Position]:
        ...
        for adj in base.adjacent_cardinals(gunner_pos):
            ...
            if base.on_core_tile(adj, self.enemy_core_pos):
                continue
            # Gunner accepts from any cardinal side EXCEPT the side it's facing
    ...
    # -----------------------------------------------------------------------
    # ── AXIONITE BACK TO OUR CORE ──────────────────────────────────────────
    #
    # Strategy:
    #   1. Find axionite ore near the enemy core.
    #   2. Build a harvester on it.
    #   3. Lay a conveyor chain pointing back toward our own core.
    #      Because the map can be large, we use a sequence of bridges (each
    #      spanning up to dist²=9) striding toward home, landing on empty
    #      tiles wherever possible.
    #   The core will receive raw axionite. Raw axionite fed to the core is
    #   converted: the core's run_core() now calls ct.convert() each turn
    #   to turn any refined axionite → titanium. Raw axionite delivered to
    #   the core is simply destroyed (per spec), so we only net benefit from
    #   this if we manage to build a foundry — but even raw axionite delivery
    #   scores on the axionite-delivered tiebreaker.
    #
    #   NOTE: If we find an enemy axionite harvester/conveyor chain near the
    #   core, we try to REDIRECT it: place a bridge from the last conveyor
    #   before our side of the map toward our own core direction.
    # -----------------------------------------------------------------------

    def try_maintain_axionite_route(self, ct: Controller) -> bool:
        """
        Try to route axionite ore (found near enemy core) back toward our core.
        Returns True if an action was taken this turn.
        """
        assert self.enemy_core_pos is not None and self.core_pos is not None
        assert self.width is not None and self.height is not None

        # Check if our existing harvester project is still on track
        if self.ax_harvester_pos is not None and ct.is_in_vision(self.ax_harvester_pos):
            bid = ct.get_tile_building_id(self.ax_harvester_pos)
            if bid is None or ct.get_entity_type(bid) != EntityType.HARVESTER:
                self.ax_harvester_pos = None
                self.ax_chain_done = False

        if self.ax_chain_done:
            return False

        if ct.get_current_round() - self.last_ax_replan_round < 10 and self.ax_harvester_pos is None:
            return False

        current = ct.get_position()

        # Step 1: find an axionite ore tile near the enemy core
        best_ax_ore: Position | None = None
        best_ax_score = 10 ** 9
        for pos in ct.get_nearby_tiles():
            if ct.get_tile_env(pos) != Environment.ORE_AXIONITE:
                continue
            if pos.distance_squared(self.enemy_core_pos) > 50:
                continue  # only near-enemy tiles are worth routing from here
            existing = ct.get_tile_building_id(pos)
            if existing is not None and ct.get_entity_type(existing) not in (
                EntityType.HARVESTER, EntityType.MARKER
            ):
                continue
        ...
        if best_ax_ore is None:
            self.last_ax_replan_round = ct.get_current_round()
            return False

        # Step 2: build a harvester on it
        if self.ax_harvester_pos is None:
            stand = self.find_build_stand(ct, best_ax_ore)
            if stand is None:
                self.last_ax_replan_round = ct.get_current_round()
                return False
            if (
                current.distance_squared(best_ax_ore) <= GameConstants.ACTION_RADIUS_SQ
                and abs(current.x - best_ax_ore.x) + abs(current.y - best_ax_ore.y) == 1
            ):
                bid = ct.get_tile_building_id(best_ax_ore)
                if bid is not None and ct.get_entity_type(bid) == EntityType.HARVESTER:
                    self.ax_harvester_pos = best_ax_ore
                else:
                    cost = ct.get_harvester_cost()
                    if base.can_afford(ct, cost[0], cost[1], reserve=base.reserve_assault(ct)) and ct.can_build_harvester(best_ax_ore):
                        ct.build_harvester(best_ax_ore)
                        self.ax_harvester_pos = best_ax_ore
                        return True
                    return False
            else:
        ...
        # Step 3: build a conveyor chain (using bridges) from the harvester
        #         toward our core, one segment at a time.
        # We walk from the harvester toward our core in bridge-length strides.
        return self._extend_axionite_chain_toward_core(ct, self.ax_harvester_pos)

    def _extend_axionite_chain_toward_core(self, ct: Controller, start: Position) -> bool:
        """
        From `start`, find the furthest conveyor/bridge already pointing toward
        our core; then extend by one more link.  Returns True if an action was taken.
        """
        assert self.core_pos is not None and self.width is not None and self.height is not None
        current = ct.get_position()

        # Walk along the existing chain starting from `start`
        cursor = start
        for _ in range(50):  # safety cap
            # Find a conveyor or bridge at cursor pointing toward home
            if not ct.is_in_vision(cursor):
                break
            bid = ct.get_tile_building_id(cursor)
            next_pos: Position | None = None
            if bid is not None:
                etype = ct.get_entity_type(bid)
                if etype == EntityType.CONVEYOR and ct.get_team(bid) == ct.get_team():
                    next_pos = cursor.add(ct.get_direction(bid))
                elif etype == EntityType.BRIDGE and ct.get_team(bid) == ct.get_team():
                    next_pos = ct.get_bridge_target(bid)
                elif etype == EntityType.HARVESTER:
                    # Start of chain is the harvester itself; pick a cardinal side as first link
                    for direction in base.CARDINAL_DIRECTIONS:
                        adj = cursor.add(direction)
                        if base.in_bounds(adj, self.width, self.height) and ct.is_in_vision(adj):
                            adj_bid = ct.get_tile_building_id(adj)
                            if adj_bid is not None:
                                adj_type = ct.get_entity_type(adj_bid)
                                if adj_type in (EntityType.CONVEYOR, EntityType.BRIDGE) and ct.get_team(adj_bid) == ct.get_team():
                                    next_pos = adj
                                    break
                    if next_pos is None:
                        # No outgoing link yet — place one
                        break
                else:
                    break
            else:
            ...
            # Have we reached the core area?
            if base.on_core_tile(next_pos, self.core_pos):
                self.ax_chain_done = True
                return False

            cursor = next_pos

        # `cursor` is now the last known tile in the chain (or the harvester)
        # We need to extend one link from `cursor` toward our core.
        # Use a bridge if possible (strides of up to sqrt(9)=3 tiles).

        if cursor == start:
            # Nothing placed yet; start with a simple conveyor off the harvester
            for direction in base.CARDINAL_DIRECTIONS:
                adj = start.add(direction)
                if not base.in_bounds(adj, self.width, self.height):
                    continue
                if base.on_core_tile(adj, self.enemy_core_pos) if self.enemy_core_pos else False:
                    continue
                if ct.get_tile_env(adj) != Environment.EMPTY and not ct.is_tile_passable(adj):
                    continue
                bid = ct.get_tile_building_id(adj)
                if bid is not None and ct.get_entity_type(bid) not in (EntityType.MARKER,):
                    continue
                # Point this conveyor toward our core
                toward_core = adj.direction_to(self.core_pos)
                if current.distance_squared(adj) > GameConstants.ACTION_RADIUS_SQ:
                    return self._assault_step_toward(ct, adj)
                cost = ct.get_conveyor_cost()
                if base.can_afford(ct, cost[0], cost[1], reserve=base.BASE_RESERVE) and ct.can_build_conveyor(adj, toward_core):
                    ct.build_conveyor(adj, toward_core)
                    return True
            return False

        # Extend from cursor toward core with a bridge or conveyor
        # Choose best landing tile: closest empty tile toward core within dist²≤9
        toward_core_dir = cursor.direction_to(self.core_pos)
        best_land: Position | None = None
        best_dist_to_core = cursor.distance_squared(self.core_pos)
        ...
        for dx in range(-3, 4):
            for dy in range(-3, 4):
                ...
                if dx == 0 and dy == 0:
                    continue
                candidate = Position(cursor.x + dx, cursor.y + dy)
                if not base.in_bounds(candidate, self.width, self.height):
                    continue
                d_to_core = candidate.distance_squared(self.core_pos)
                if d_to_core >= best_dist_to_core:
                    continue  # must make progress
                if not ct.is_in_vision(candidate):
                    continue
                if ct.get_tile_env(candidate) != Environment.EMPTY and not ct.is_tile_passable(candidate):
                    continue
                bid = ct.get_tile_building_id(candidate)
                if bid is not None:
                    etype = ct.get_entity_type(bid)
                    # Allow overwriting markers; skip occupied tiles otherwise
                    if etype not in (EntityType.MARKER,):
                        continue
                if best_land is None or d_to_core < best_land.distance_squared(self.core_pos):
                    best_land = candidate
        ...
        # Move into build range of cursor (we build at cursor, bridging to best_land)
        if current.distance_squared(cursor) > GameConstants.ACTION_RADIUS_SQ:
            return self._assault_step_toward(ct, cursor)

        is_diagonal = cursor.x != best_land.x and cursor.y != best_land.y
        if is_diagonal or cursor.distance_squared(best_land) > 1:
            cost = ct.get_bridge_cost()
            if base.can_afford(ct, cost[0], cost[1], reserve=base.BASE_RESERVE):
                if ct.can_build_bridge(cursor, best_land):
                    ct.build_bridge(cursor, best_land)
                    return True
        else:
            toward = cursor.direction_to(best_land)
            cost = ct.get_conveyor_cost()
            if base.can_afford(ct, cost[0], cost[1], reserve=base.BASE_RESERVE):
                # Place conveyor AT cursor pointing to best_land — but cursor
                # may already have the harvester.  Place at best_land instead.
                if ct.can_build_conveyor(best_land, best_land.direction_to(self.core_pos)):
                    ct.build_conveyor(best_land, best_land.direction_to(self.core_pos))
                    return True
