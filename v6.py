...
TARGET_FOUNDRIES = 2
EXTRA_SPAWN_SURPLUS = 180
EXTRA_SPAWN_COOLDOWN = 10
OFFENSE_MIN_ROUND = 100
...
class Player:
    ...
    def feeder_should_build_foundry(self) -> bool:
        index = self.feeder_index()
        return index is not None and index < self.target_foundry_count()

    def defense_ready(self, ct: Controller) -> bool:
        if self.count_foundries(ct) < self.target_foundry_count():
            return False
        titanium, _ = ct.get_global_resources()
        return (
    ...
    def offense_ready(self, ct: Controller) -> bool:
        ...
        if self.count_foundries(ct) < self.target_foundry_count():
            return False
        if ct.get_current_round() < OFFENSE_MIN_ROUND and axionite < 80:
            return False
    ...
    def siege_positions(self) -> tuple[list[Position], list[Position]]:
        ...
        barrier_candidates = [
            Position(self.enemy_core_pos.x + dx, self.enemy_core_pos.y - 2)
            for dx in (-1, 0, 1)
        ] + [
            Position(self.enemy_core_pos.x + dx, self.enemy_core_pos.y + 2)
            for dx in (-1, 0, 1)
        ] + [
            Position(self.enemy_core_pos.x - 2, self.enemy_core_pos.y + dy)
            for dy in (-1, 0, 1)
        ] + [
            Position(self.enemy_core_pos.x + 2, self.enemy_core_pos.y + dy)
            for dy in (-1, 0, 1)
    ...
    def _assign_feeder_mission(self, ct: Controller) -> None:
        ...
        should_build_foundry = self.feeder_should_build_foundry()
